...
def make_frozen(cfg: Config, D: int) -> Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray]:
    ...
    The frozen attractor's sigma is read from cfg.frozen_sigma so it can
    be tuned per dataset.  Phase A revision: this used to be hardcoded to
    2.0 * image_scale, which left the frozen attractors essentially inert
    once the data was further than 3*sigma from them.
    """
    qs = np.stack([cfg.q_star(lab, D) for lab in cfg.class_labels]).astype(np.float32)
    C = qs.shape[0]
    w = jnp.full((C,), -2.0, dtype=jnp.float32)
    mu = jnp.asarray(qs)
    sigma = jnp.full((C,), float(cfg.frozen_sigma), dtype=jnp.float32)
    return w, mu, sigma
